...
- The @Injectable() decorator tells Angular that this service might
...
** DOM
|----------------------+-----------------------+-----------------------------------------------+-----------------------|
| *DOM Operations*     | *Methods*             | *Tools*                                       | *Built-in Directives* |
|----------------------+-----------------------+-----------------------------------------------+-----------------------|
| Change/Read DOM      | =classList.add()=,    | Sandboxed DOM access through =Renderer2=,     | NgClass,              |
| element properties   | =setAttribute()=,     | Don't use =Renderer2= to change DOM hierarchy | NgStyle               |
|                      | =style.setProperty()= |                                               |                       |
|----------------------+-----------------------+-----------------------------------------------+-----------------------|
| Modify DOM Hierarchy | =createElement()=,    | Templating through =ViewContainerRef=         | ngIf,                 |
|                      | =remove()=,           |                                               | ngFor,                |
|                      | =appendChild()=,      |                                               | ngSwitch              |
|                      | =removeChild()=       |                                               |                       |
|----------------------+-----------------------+-----------------------------------------------+-----------------------|
** From Documentation
- Expression guidelines
  + No visible side effects
    A template expression should not change any application state
    other than the value of the target property.
  + Quick execution
    Angular executes template expressions after every change detection cycle.
  + Simplicity
  + Idempotence
    In Angular terms, an idempotent expression always returns exactly
    the same thing until one of its dependent values changes.
    Dependent values should not change during a single turn of the
    event loop. If an idempotent expression returns a string or a
    number, it returns the same string or number when called twice in
    a row. If the expression returns an object (including an array),
    it returns the same object reference when called twice in a row.

*** HTML attribute vs. DOM property
The distinction between an HTML attribute and a DOM property is
crucial to understanding how Angular binding works.
...
cannot enable a button by writing <button disabled="false">Still
Disabled</button>.
...
*This fact bears repeating: Template binding works with properties and
