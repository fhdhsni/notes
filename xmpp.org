...
- The following stanza requests Elizabeth’s roster from her server:
  ...
  + Her server will reply with something similar to the following
    ...
    * The ~subscription~ attribute is set based on the presence
      subscription status of the contact. It can have the value ~both~
      if there is a subscription in each direction, ~to~ if Elizabeth
      has a subscription but the other party does not, and ~from~ if
      Elizabeth is not subscribed to the contact’s presence but the
      other user has a subscription to her presence. Generally, the
      user will only want to see contacts in the roster for which the
      subscription value is ~both~ or ~to~.
