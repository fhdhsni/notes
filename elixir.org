...
* Elixir
Most of these notes are from official elixir guide and [[learnelixir.tv][learnelixir.tv]]
...
15) In Elixir when can compare two different types. the order is
...
*** Bitwise Operators
*** Logical Operators
- Elixir provides three boolean operators: =or=, =and= and
...
a later time. Sometimes it is useful to run a task to compute a value
and read its result later on. For this, tasks also provide the
async/await pattern:
