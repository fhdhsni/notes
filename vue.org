...
=el=, as you might expect, is the element the binding sits on. =binding=
is an object which contains arguments that are passed into the
hooks. There are many available arguments, including =name=, =value=,
=oldValue=, =expression=, =arg=, and =modifiers=. =vnode= has a more unusual
use-case, it's available in case you need to refer directly to the
node in the virtual DOM. Both =binding= and =vnode= should be treated as
read-only.
...
#+BEGIN_SRC html
  <div class="box" v-scroll="handleScroll">
    <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. A atque
    sequi quisquam.</p>
  </div>
#+END_SRC
